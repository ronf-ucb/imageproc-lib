...
static char temp_buffer[255];  // temporary to see if serial is being read correctly
...
void __attribute__((__interrupt__, no_auto_psv)) _U2RXInterrupt(void) {
	...
	// state IDLE: init checksum to byte length, set state CHECK_SIZE
	...
	{   rx_checksum = rx_byte;
		...
		if (rx_idx == rx_payload->data_length + PAYLOAD_HEADER_LENGTH) 
